...
class RightClickSelection(tk.Menu):
    ...
    def __init__(self, master):
        ...
        self.add_command(label='Delete Selection',
                         command=lambda: self.master.delete_selection())
...
class TreeView(ttk.LabelFrame):
    ...
    def __init__(self, master, lf_title, column):
        ...
        self.tv_frame.grid(sticky='nsew', pady=5, padx=5,
                           row=1, column=0, ipady=5, ipadx=5)
        ...
        self.tree = ttk.Treeview(
            self.tv_frame, columns=column, show='headings')
        ...
        [self.tree.heading(col, text=col, anchor=CENTER, command=lambda _col=col: self.sort_column(
            self.tree, _col, False)) for col in column]
        ...
        self.load_button = ttk.Button(
            self.load_frame, text='Load', state=tk.DISABLED)
        ...
        self.convert_button = ttk.Button(
            self.converted_frame, text='Convert', state=tk.DISABLED)
