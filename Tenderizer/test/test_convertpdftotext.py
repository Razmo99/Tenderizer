...
class TestRegexEntry(TKinterTestCase, PDFToTextTestCase):
    ...
    * Pdf
    ...
    def new_convertpdftotext(self):
        ...
        self.convertpdftotext.input.entry.insert(
            tk.END, self.input_dir.resolve())
        self.convertpdftotext.input.assert_dir()
        self.convertpdftotext.output.entry.insert(
            tk.END, self.output_dir.resolve())
    ...
    def test_convert_pdfs(self):
        ...
        self.assertEqual(
            covnert_pdftotext.get_convert_state().string, tk.NORMAL)

        covnert_pdftotext.import_pdfs()
        self.pump_events()
        self.assertEqual(
            [i.name for i in covnert_pdftotext.dataset], self.pdfs)
    ...
    def test_except_halts_conversion__then_log_and_raise(self):
        ...
        with self.assertLogs('app_tk_widgets.frames.convertpdftotext', level=logging.ERROR) as cm:
            ...
            self.assertEqual(cm.output, [
                             f'ERROR:app_tk_widgets.frames.convertpdftotext:Error opening a PDF file: "{del_pdf.resolve()}"'])
        self.pump_events()
        self.assertNotIn(
            True, [i.converted for i in covnert_pdftotext.dataset])
